module RecordCache
  ...
  private
    ...
    def in_clause(keys)
      ...
      if keys.any?
        values = keys.collect do |key|
          key = unescape_spaces(key)
          quote_index_value(key)
        end.join(',')
    ...
    def stringify(keys)
      ...
      keys.collect {|key| key.nil? ? NULL : escape_spaces(key)}.uniq
    end

    def escape_spaces(key)
      key.to_s.gsub(/%/, '%%').gsub(/ /, '%s')
    end

    def unescape_spaces(key)
